...
def reconstruct_field(field_tp, datai, mask_tp, mask_cv, neof):
    ...
    while e[i] < e[i-1]:
        ...
        eigv, eigval, eigvt = np.linalg.svd(np.cov(field.T),
                                            full_matrices=True)
...
def reconstruct_per_EOF(field_tp, datai, mask_tp, mask_cv, neof):
    fields = []
    rms_cv = []
