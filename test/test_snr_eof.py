...
tirage = 100
for ti in range(tirage): # loop for gap generation type
...
cmap = plt.cm.RdGy
rcParams['axes.prop_cycle'] = cycler(color=cmap(np.linspace(0, 1, tirage)))
fig, ax = plt.subplots()
rms = np.array(rms)
rms = np.reshape(rms,(tirage,inter)).T
lines = ax.plot(sn_ratio[:inter], rms, alpha=0.4)
#ax.legend(lines)
plt.show()
